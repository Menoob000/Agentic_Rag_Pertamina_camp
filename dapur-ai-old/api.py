...
from RAG.store import ingest_pdf_documents
...
@app.get("/")
async def root():
    return {"status": "success", "message": "API is running. Welcome to Pertamina RKS AI System - Dapur AI."}
...
async def ingest_pdf(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    ...
    # Save the file to RAG/sample_data
    save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "RAG", "sample_data")
    ...
    background_tasks.add_task(ingest_pdf_documents)
